...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        config.pool_config.connection_timeout_ms
    );
    println!(
        "     Idle timeout: {}ms",
        config.pool_config.idle_timeout_ms
    );
    println!(
        "     Max lifetime: {}ms\n",
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let result = client_clone.data().query("SELECT 1 as value").await;
    ...
    println!(
        "     Active connections: {}",
        metrics.connection_metrics.active_connections
    );
    println!(
        "     Idle connections: {}",
        metrics.connection_metrics.idle_connections
    );
    println!(
        "     Total connections: {}",
        metrics.connection_metrics.total_connections
    );
    println!(
        "     Connection errors: {}",
        metrics.connection_metrics.connection_errors
    );
    println!(
        "     Connection timeouts: {}\n",
    ...
    println!(
        "     Min latency: {:.2}ms",
        metrics.query_metrics.min_latency_ms
    );
    println!(
        "     Max latency: {:.2}ms",
        metrics.query_metrics.max_latency_ms
    );
    println!(
        "     Avg latency: {:.2}ms",
        metrics.query_metrics.avg_latency_ms
    );
    println!(
        "     P50 latency: {:.2}ms",
        metrics.query_metrics.percentiles.p50
    );
    println!(
        "     P95 latency: {:.2}ms",
        metrics.query_metrics.percentiles.p95
    );
    println!(
        "     P99 latency: {:.2}ms\n",
