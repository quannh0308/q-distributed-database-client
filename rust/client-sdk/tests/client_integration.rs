...
async fn test_client_health_check() {
    ...
    assert!(health.healthy_nodes > 0, "Should have at least one healthy node");
...
async fn test_client_authentication_failure() {
    ...
    assert!(result.is_err(), "Connection with invalid credentials should fail");
}
